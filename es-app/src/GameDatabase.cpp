...
bool GameDatabase::init(const std::string& dbPath)
{
	...
	exec("PRAGMA cache_size=-65536;"); // 64  MB
	exec("PRAGMA mmap_size=268435456;"); // 256 MB
	exec("PRAGMA optimize;"); // 256 MB
	...
	LOG(LogDebug) << "GameDatabase: Opened database at " << dbPath;
	return true;
}
...
void GameDatabase::deinit()
{
	if (mDb != nullptr)
	{
		sqlite3_close(mDb);
		mDb = nullptr;
		LOG(LogDebug) << "GameDatabase: Closed database";
	}
}
...
bool GameDatabase::createTables()
{
	...
	if (stmt)
	{
		if (sqlite3_step(stmt) == SQLITE_ROW)
		{
			...
			{
				LOG(LogDebug) << "GameDatabase: Schema version changed (" << version << " -> " << CURRENT_SCHEMA_VERSION << "), rebuilding cache";
...
bool GameDatabase::hasSystemCache(const std::string& systemName)
{
	...
	sqlite3_stmt* stmt = prepare("SELECT EXISTS(SELECT 1 FROM games WHERE system = ?)");
	if (!stmt)
		return false;
...
static inline std::string getColumnText(sqlite3_stmt* stmt, int col)
{
...
static void jsonToMetadata(const std::string& json, MetaDataList& md, SystemData* system)
{
	...
	static const std::unordered_map<std::string_view, int> keyToId = []()
	{
		std::unordered_map<std::string_view, int> m;

		for (const auto& decl : MetaDataList::getMDD())
			m.emplace(decl.key, decl.id);
	...
	struct Handler : public rapidjson::BaseReaderHandler<rapidjson::UTF8<>, Handler>
	{
		...
		explicit Handler(MetaDataList& m) : md(m) { }

		bool Key(const char* str, rapidjson::SizeType len, bool /*copy*/)
		{
			currentId = -1;

			auto it = keyToId.find(std::string_view(str, len));
			if (it != keyToId.end())
				currentId = it->second;
		...
		{
			if (currentId != -1 && len > 0)
				md.set((MetaDataId) currentId, std::string(str, len));
...
bool GameDatabase::loadSystem(SystemData* system, std::unordered_map<std::string, FileData*>& fileMap)
{
	...
	sqlite3_stmt* stmt = prepare("SELECT path, type, metadata FROM games WHERE system = ? ORDER BY path");
	if (!stmt)
		return false;
	...
	LOG(LogDebug) << "GameDatabase: Loaded " << count << " games for " << systemName << " from cache";
	return count > 0;
}
										...
										std::unordered_map<std::string, FileData*>& fileMap)
{
	...
	if (type == GAME && !system->getSystemEnvData()->isValidExtension(Utils::String::toLower(Utils::FileSystem::getExtension(path))))
		return nullptr;
...
bool GameDatabase::syncSystem(SystemData* system)
{
	...
	LOG(LogDebug) << "GameDatabase: Synced " << games.size() << " games for " << systemName;
	return true;
}
...
int GameDatabase::removeStaleGames(const std::string& systemName, const std::vector<std::string>& validPaths)
{
	...
	if (removed > 0)
		LOG(LogDebug) << "GameDatabase: Removed " << removed << " stale games for " << systemName;
